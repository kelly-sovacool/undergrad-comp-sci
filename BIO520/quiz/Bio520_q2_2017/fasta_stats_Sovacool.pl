...
# counts each base in the current line
            ...
            $xcount += tr/xX/xX/;
			$sequence_length=$acount + $ccount + $gcount + $tcount + $ncount + $xcount;
			$g_plus_c = $gcount + $ccount;
			$known_count = $g_plus_c + $acount + $tcount;
			# exclude unknown bases from GC content calculation
			$percent_gc = 100 * $g_plus_c / $known_count;
			print OUTFILE ("$Sequence_name\t$percent_gc\t$sequence_length\n");
		}
        $line = <INFILE2>;
     }
$sequence_length=$acount + $ccount + $gcount + $tcount + $ncount + $xcount;
