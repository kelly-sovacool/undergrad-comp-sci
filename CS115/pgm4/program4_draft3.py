...
def selection_sort(domain_list, ip_list, sort_var):
     ...
     if sort_var == 'Domain':
          ...
          for i in range(len(domain_list)):
               ...
               old_pos = old_dom_list.index(domain_list[i])
               # move the corresponding ip to its new index in the sorted list
               ip_list[i] = old_ip_list[old_pos]
     # else if user sorts by IP:
          ...
          for i in range(len(ip_list)):
               # find the position of the element in the unsorted list
               old_pos = old_ip_list.index(ip_list[i])
               # assign the corresponding domain to its new index in the sorted lsit
               domain_list[i] = old_dom_list[old_pos]
...
def display_lists(domain_list, ip_list):
     ...
     '''
     # Design:
     # Determine how many spaces are needed between 'Domain' and 'IP' headers for formatting
     largest_len = len('Domain Name')
     for ele in domain_list:
          if len(ele) > largest_len:
               largest_len = len(ele)
     ...
     for i in range(len(domain_list)):
          spaces = ' ' * (largest_len - len(domain_list[i]))
          # print the domain and its corresponding ip on the same line
          print(domain_list[i], spaces, ip_list[i])
     print('--------------------------------------------------')
main()